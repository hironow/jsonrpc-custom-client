...
export function MessageDetailSidebar({ message, onClose, linkedMessage }: MessageDetailSidebarProps) {
  ...
  if (isBatchMessage) {
    ...
    if (message.type === "sent") {
      ...
      batchResponses = linkedMessage && Array.isArray(linkedMessage.data) ? linkedMessage.data : []
    } else {
      // Current message is the batch response
      batchRequests = linkedMessage && Array.isArray(linkedMessage.data) ? linkedMessage.data : []
    ...
    return (
      <div className="space-y-6">
        {batchPairs.map((pair) => {
          ...
          const isNotification = request.id === undefined && request.method !== undefined
            ...
            <div key={index} className="border border-border rounded-lg overflow-hidden">
              ...
              <div className="p-4 border-b border-border">
                ...
                <div className="space-y-2 mb-3">
                  {request.method && (
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span className="font-mono text-blue-400 font-semibold">{request.method}</span>
                    </div>
                  )}
                ...
                {request.params !== undefined ? (
                  <div>
                    <h4 className="text-xs font-semibold text-foreground mb-1.5">Parameters</h4>
              ...
              {hasResponse ? (
                <>
                  ...
                  {latency !== undefined && (
                    <div className="py-2 px-4 bg-muted/30 border-b border-border flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[10px] text-muted-foreground font-medium">Response Time</div>
                        <div className={`text-sm font-bold font-mono ${getLatencyColor(latency)}`}>
                          {latency}
                  ...
                  <div className="p-4">
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => copyToClipboard(response, `batch-res-${index}`)}
                      className="w-full mb-3 h-7 text-xs"
                    >
                    ...
                    {response.result !== undefined ? (
                      <div>
                        <h4 className="text-xs font-semibold text-foreground mb-1.5">Result</h4>
                        {renderData(response.result, "received")}
                      </div>
                    ) : response.error !== undefined ? (
                      <div>
                        <h4 className="text-xs font-semibold text-destructive mb-1.5">Error</h4>
                ...
                <div className="p-4 flex flex-col items-center justify-center bg-muted/20 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">No response expected for notification</p>
                </div>
              ) : (
                <div className="p-4 flex flex-col items-center justify-center bg-muted/20 text-center">
  ...
  const renderWaitingForResponse = () => {
    return (
      <div className="flex flex-col h-full items-center justify-center p-6 bg-muted/20">
        ...
        <p className="text-sm font-medium text-foreground mb-1">Waiting for Response...</p>
        <p className="text-xs text-muted-foreground text-center">The response will appear here when it arrives</p>
      </div>
    )
  }
      ...
      <Card className="flex flex-col h-full bg-card overflow-hidden">
        <div className="p-3 border-b border-border flex-shrink-0 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-foreground">Batch Message Details</h2>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4">
          <div className="space-y-2 mb-4 pb-4 border-b border-border">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-mono text-foreground">{formatTime(message.timestamp)}</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-muted-foreground">Date:</span>
              <span className="text-foreground">{formatDate(message.timestamp)}</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
          ...
          {(message.validationErrors || message.validationWarnings) && <div className="mb-4" />}
  ...
  return (
    <Card className="flex flex-col h-full bg-card overflow-hidden">
      <div className="p-3 border-b border-border flex-shrink-0 flex items-center justify-between">
        <h2 className="text-sm font-semibold text-foreground">Message Details</h2>
        <Button
          variant="ghost"
          size="sm"
          onClick={onClose}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="p-4 border-b border-border">
            ...
            <div className="space-y-2 mb-3">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">Time:</span>
                <span className="font-mono text-foreground">{formatTime(requestMessage!.timestamp)}</span>
              </div>
              {requestMessage!.method && (
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-muted-foreground">Method:</span>
                  <span className="font-mono text-blue-400 font-semibold">{requestMessage!.method}</span>
                </div>
              )}
            </div>
            {renderValidationResults(requestMessage!)}
            {(requestMessage!.validationErrors || requestMessage!.validationWarnings) && <div className="mb-3" />}
            <Button
              variant="outline"
              size="sm"
          ...
          {latency !== undefined && (
            <div className="py-3 px-4 bg-muted/30 border-b border-border flex items-center gap-3">
              ...
              <div>
                <div className="text-[10px] text-muted-foreground font-medium">Response Time</div>
                <div className={`text-lg font-bold font-mono ${getLatencyColor(latency)}`}>
                  {latency}
                  <span className="text-xs ml-1">ms</span>
                </div>
              </div>
          ...
          <div className="p-4">
            ...
            <div className="space-y-2 mb-3">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">Time:</span>
                <span className="font-mono text-foreground">{formatTime(responseMessage!.timestamp)}</span>
              </div>
            </div>
            {renderValidationResults(responseMessage!)}
            {(responseMessage!.validationErrors || responseMessage!.validationWarnings) && <div className="mb-3" />}
            <Button
              variant="outline"
              size="sm"
          ...
          <div className="p-4 border-b border-border">
            ...
            <div className="space-y-2 mb-3">
              ...
              {message.method && (
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-muted-foreground">Method:</span>
                  <span className="font-mono text-blue-400 font-semibold">{message.method}</span>
                </div>
              )}
            </div>
            {renderValidationResults(message)}
            {(message.validationErrors || message.validationWarnings) && <div className="mb-3" />}
            <Button
              variant="outline"
              size="sm"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="p-4 border-b border-border">
            ...
            <div className="space-y-2 mb-3">
              <div className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">Time:</span>
                <span className="font-mono text-foreground">{formatTime(linkedMessage.timestamp)}</span>
              </div>
              {linkedMessage.method && (
                <div className="flex items-center gap-2 text-xs">
                  <span className="text-muted-foreground">Method:</span>
                  <span className="font-mono text-blue-400 font-semibold">{linkedMessage.method}</span>
                </div>
              )}
            </div>
            {renderValidationResults(linkedMessage)}
            {(linkedMessage.validationErrors || linkedMessage.validationWarnings) && <div className="mb-3" />}
            <Button
              variant="outline"
              size="sm"
