...
export function ConnectionQualityMonitor({
	...
	const connectionStats = useMemo(() => {
		...
		// Calculate uptime
		let totalUptime = 0;
		let lastConnectMs: number | null = null;

		events.forEach((event) => {
			if (event.type === "connected") {
				lastConnectMs = event.timestamp.getTime();
			} else if (event.type === "disconnected" && lastConnectMs !== null) {
				totalUptime += event.timestamp.getTime() - lastConnectMs;
				lastConnectMs = null;
			}
		});

		// If currently connected, add current session time
		if (status === "connected" && lastConnectMs !== null) {
			totalUptime += Date.now() - lastConnectMs;
		}

		// Calculate average latency from response messages
		const responseTimes = messages
			.filter(
				(msg) => msg.type === "received" && msg.responseTime !== undefined,
			)
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Monitor WebSocket connection health and stability
				</CardDescription>
			</CardHeader>
			<CardContent>
				<div className="space-y-4">
					...
					<div className="space-y-2">
						...
						<div className="flex items-center gap-3">
							<Progress
								value={connectionStats.qualityScore}
								className="flex-1"
							/>
							<span
								className={`text-2xl font-bold ${getQualityColor(connectionStats.qualityScore)}`}
							>
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<div className="space-y-1 p-3 rounded-lg bg-muted/50">
							...
							<div className="text-lg font-bold">
								{connectionStats.avgLatency.toFixed(0)}ms
							</div>
						</div>
					</div>
					...
					<div className="space-y-1 p-3 rounded-lg bg-muted/50">
						...
						<div className="text-lg font-bold">
							{formatUptime(connectionStats.totalUptime)}
						</div>
					</div>
