...
export function MessageItem({
		...
		if (message.isBatch) {
			if (message.type === "sent") {
				...
				const hasError =
					...
					message.data.some((item: any) => item.error);
				return {
					icon: (
						<div className="relative">
	...
	const getResponseTimeDisplay = () => {
		...
		return (
			<span
				className={`text-[9px] font-mono ${colorClass} flex items-center gap-0.5`}
			>
	...
	const getInlinePreview = () => {
		if (message.type === "system" || typeof message.data !== "object")
			return null;

		if (message.isBatch && Array.isArray(message.data)) {
			const elements = [];

			const successCount = message.data.filter(
				(item: any) => item.result !== undefined,
			).length;
			const errorCount = message.data.filter(
				(item: any) => item.error !== undefined,
			).length;
			...
			if (message.isPending) {
				elements.push(
					<Badge
						key="pending"
						variant="secondary"
						className="text-[9px] px-1 py-0 h-3.5 animate-pulse"
					>
						Pending...
					</Badge>,
				);
			...
			if (message.validationErrors && message.validationErrors.length > 0) {
				elements.push(
					...
					>
						...
						{message.validationErrors.length > 1 ? "s" : ""}
					</Badge>,
				);
			}
			if (message.validationWarnings && message.validationWarnings.length > 0) {
				elements.push(
					...
					>
						...
						{message.validationWarnings.length} warning
						{message.validationWarnings.length > 1 ? "s" : ""}
					</Badge>,
				);
			}
			...
			return (
				<div className="flex items-center gap-1 flex-wrap mt-0">{elements}</div>
			);
		}
			...
			elements.push(
				...
				>
					...
					<span className="text-[9px]">
						{message.type === "sent" ? "→ Response" : "← Request"}
					</span>
				</span>,
			);
		...
		if (data.params) {
			if (Array.isArray(data.params)) {
				const preview = data.params.slice(0, 3).map((p: any) => {
					if (typeof p === "string")
				...
				const preview = entries.map(([k, v]) => {
					let val = v;
					if (typeof v === "string")
						val = `"${v.length > 15 ? v.slice(0, 15) + "..." : v}"`;
	...
	const linkColor = message.data?.method
		? getMethodColor(message.data.method)
		: "#888";
