...
export function ResponseTimeHeatmap({ messages }: ResponseTimeHeatmapProps) {
	const calculateHeatmapData = (): HeatmapCell[] => {
		const cells: Map<string, { totalLatency: number; count: number }> =
			new Map();
		...
		messages.forEach((msg) => {
			const latency = msg.responseTime as number | undefined;
			const timestamp =
				msg.timestamp instanceof Date
					? msg.timestamp.getTime()
					: (msg.timestamp as any as number);
	...
	const methods = Array.from(
		new Set(heatmapData.map((cell) => cell.method)),
	).sort();
	const timeBuckets = Array.from(
		new Set(heatmapData.map((cell) => cell.timeBucket)),
	).sort((a, b) => a - b);
	...
	const getCellData = (
		...
		return heatmapData.find(
			(cell) => cell.method === method && cell.timeBucket === timeBucket,
		);
	};
		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Visualize response times by method and time
					</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="text-sm text-muted-foreground text-center py-8">
						No response data available yet. Send some requests to see the
						heatmap.
					</div>
				</CardContent>
			</Card>
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Last hour - Response times by method and time (5-minute intervals)
				</CardDescription>
			</CardHeader>
			<CardContent>
													...
													<>
														...
														<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
															<div className="bg-popover text-popover-foreground border rounded-lg shadow-lg p-2 text-xs whitespace-nowrap">
																...
																<div className="mt-1">
																	Avg: {cellData.avgLatency.toFixed(0)}ms
																</div>
