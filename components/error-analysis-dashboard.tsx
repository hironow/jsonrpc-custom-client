...
export function ErrorAnalysisDashboard({
	...
	const getValidationErrorStats = () => {
		const totalMessages = messages.filter(
			(m) => m.type === "sent" || m.type === "received",
		).length;
		const messagesWithErrors = messages.filter(
			(m) => m.validationErrors && m.validationErrors.length > 0,
		).length;
		const messagesWithWarnings = messages.filter(
			(m) => m.validationWarnings && m.validationWarnings.length > 0,
		).length;
		...
		return {
			...
			errorRate:
				totalMessages > 0 ? (messagesWithErrors / totalMessages) * 100 : 0,
			warningRate:
				totalMessages > 0 ? (messagesWithWarnings / totalMessages) * 100 : 0,
		};
	};

	const errorStats = getErrorStats();
	const validationStats = getValidationErrorStats();
	const sortedStats = Array.from(errorStats.values()).sort(
	...
	const totalCalls = Array.from(errorStats.values()).reduce(
		...
		<Card className="p-4 bg-card">
			...
			<div className="grid grid-cols-2 gap-2 mb-4">
				<div className="bg-muted/30 rounded p-2">
					...
					<div
						className={`text-lg font-bold font-mono ${getErrorRateColor(overallErrorRate)}`}
					>
				...
				<div className="bg-muted/30 rounded p-2">
					...
					<div
						className={`text-lg font-bold font-mono ${getErrorRateColor(validationStats.errorRate)}`}
					>
						{(100 - validationStats.errorRate).toFixed(1)}%
					</div>
					<div className="text-[9px] text-muted-foreground">
						{validationStats.messagesWithErrors} errors,{" "}
						{validationStats.messagesWithWarnings} warnings
					</div>
				</div>
			</div>
				...
				<div className="space-y-2">
					...
					{sortedStats.slice(0, 5).map((stat) => (
						<div key={stat.method} className="space-y-1">
							<div className="flex items-center justify-between text-xs">
								<span className="font-mono text-blue-400 truncate flex-1">
									{stat.method}
								</span>
								<span
									className={`font-mono font-semibold ml-2 ${getErrorRateColor(stat.errorRate)}`}
								>
					...
					{sortedStats.length > 5 && (
						<p className="text-[9px] text-muted-foreground text-center pt-1">
							+{sortedStats.length - 5} more methods
						</p>
					)}
				</div>
			)}
