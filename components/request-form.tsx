...
export function RequestForm({ disabled, onSend, onSendBatch }: RequestFormProps) {
  ...
  const handleSendBatch = () => {
    ...
    const updatedRequests = batchRequests.map((req) => {
      try {
        ...
        if (!req.method) {
          return { ...req, paramsError: "Method is required" }
        }
  ...
  const addBatchRequest = () => {
    setBatchRequests([...batchRequests, { id: crypto.randomUUID(), method: "", params: "{}", paramsError: "" }])
  }
  ...
  const updateBatchRequest = (id: string, field: "method" | "params", value: string) => {
    setBatchRequests(batchRequests.map((req) => (req.id === id ? { ...req, [field]: value, paramsError: "" } : req)))
  }
  ...
  const formatBatchJSON = (id: string) => {
    ...
    try {
      ...
      setBatchRequests(batchRequests.map((r) => (r.id === id ? { ...r, paramsError: "Invalid JSON format" } : r)))
    }
  }
    ...
    <Card className="p-2 bg-card border-border flex-1">
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          {batchMode && (
            <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4 bg-purple-500/20 text-purple-400">
      ...
      <div className="flex items-center gap-2 mb-2 p-1.5 bg-muted/50 rounded">
        <Switch id="batch-mode" checked={batchMode} onCheckedChange={setBatchMode} disabled={disabled} />
        <Label htmlFor="batch-mode" className="text-xs text-muted-foreground cursor-pointer">
          Batch Mode
        </Label>
      </div>
      ...
      {!batchMode ? (
        <div className="space-y-1.5">
          <div>
            <label className="text-xs font-medium text-foreground mb-0.5 block">Method</label>
            <Input
              value={method}
          ...
          <div>
            <div className="flex items-center justify-between mb-0.5">
              <label className="text-xs font-medium text-foreground">Parameters (JSON)</label>
              <Button
                variant="ghost"
                size="sm"
                onClick={formatJSON}
            ...
            {batchRequests.map((req, index) => (
              <Card key={req.id} className="p-1.5 bg-muted/30 border-border">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-medium text-muted-foreground">Request #{index + 1}</span>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <div className="space-y-1">
                  <Input
                    value={req.method}
                    onChange={(e) => updateBatchRequest(req.id, "method", e.target.value)}
                    placeholder="Method"
                    disabled={disabled}
                  ...
                  <div>
                    ...
                    <Textarea
                      value={req.params}
                      onChange={(e) => updateBatchRequest(req.id, "params", e.target.value)}
                      placeholder="{}"
                      disabled={disabled}
                      className="bg-input border-border text-foreground font-mono text-[10px] min-h-[50px] resize-none"
                    />
                    {req.paramsError && <p className="text-[10px] text-destructive mt-0.5">{req.paramsError}</p>}
                  </div>
                </div>
              </Card>
            ))}
