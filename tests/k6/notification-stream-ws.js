...
export default function () {
  ...
  const res = ws.connect(url, { tags: { scenario: 'notification-stream' } }, function (socket) {
    let got = false;
    ...
    socket.on('message', function (data) {
      ...
      try {
        ...
        const notif = arr.find(
          (m) => m && typeof m === 'object' && !Object.prototype.hasOwnProperty.call(m, 'id') && m.method === 'stream.heartbeat',
        );
        if (!notif) return;
        check(notif, {
          'heartbeat notification': (v) => v && v.method === 'stream.heartbeat',
        });
        got = true;
        socket.close();
      } catch (_) {
