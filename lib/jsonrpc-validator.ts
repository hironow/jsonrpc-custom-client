	...
	if (hasError) {
		if (typeof data.error !== "object" || data.error === null) {
			...
			if (
				...
				errors.push('"error.code" must be an integer number');
			}
			...
			if (typeof code === "number" && Number.isInteger(code)) {
				...
				if (code <= -32000 && code >= -32099) {
					warnings.push(
						'"error.code" is in reserved server error range (-32099..-32000)',
					);
				} else if (code <= -32000 && code >= -32768) {
					// Other reserved pre-defined codes
					warnings.push('"error.code" is a reserved pre-defined code');
				}
			}
		}
	}
